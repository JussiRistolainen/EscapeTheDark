...
class Item_control:
    ...
    def create_log(self, number):
        for i in range(number):
            ...
            self.object_list.append(Burn(self.window, random_x, random_y, "Log", [random_x - 40, random_y - 18, random_x + 45, random_y + 12], [self.log_key, self.images[1][2]]))

    def create_fire(self, pos):
        obj = Flame(self.window, pos[0], pos[1], "bondfire", [pos[0] - 38, pos[1] - 25, pos[0] + 27, pos[1]], [self.fire_key, self.images[1][2]], 3, 0)
    ...
    def create_log_with_pos(self, pos):
        ...
        for i in self.object_list:
            if i.box[0] < pos[0] < i.box[2] and i.box[1] < pos[1] < i.box[3] and i.name == 'bondfire':
                ...
                picked = False
        if picked:
            self.object_list.append(Burn(self.window, pos[0], pos[1], "Log", [pos[0] - 40, pos[1] - 18, pos[0] + 45, pos[1] + 12],
                                    [self.images[0]['spritelog0'], self.images[1][2]]))
            ...
            if picked:
                if (i.box[0] < pos[0] < i.box[2]) and (i.box[1] < pos[1] < i.box[3]):
                    if i.name == "Log":
                        ...
                        self.object_list.append(Flame(self.window, pos[0], pos[1], "bondfire", [pos[0] - 38, pos[1] - 22, pos[0] + 27, pos[1]], [self.fire_key, 120], 1, 0))
                        self.fire_list.append(Flame(self.window, pos[0], pos[1], "bondfire", [pos[0] - 38, pos[1] - 22, pos[0] + 27, pos[1]], [self.fire_key, 120], 1, 0))
                        picked = False
